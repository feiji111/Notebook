...
Container是由Image创建而来的，images有两个非常重要的特点：
...
在容器的union filesystem中，除了每一层image layer都有一个目录，对于每一个从当前镜像创建的容器，都有一个对应的目录。这就使得运行的container可以修改文件系统，但是同时又不会修改image layers，使得同一个镜像可以运行多个容器。
